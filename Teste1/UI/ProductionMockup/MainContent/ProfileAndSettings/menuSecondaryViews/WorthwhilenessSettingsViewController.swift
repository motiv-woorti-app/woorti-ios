...
class WorthwhilenessSettingsViewController: GenericViewController {
    ...
    override func viewDidLoad() {
        ...
        if let productiveScaleView = nib.instantiate(withOwner: self, options: nil).first as? OnboardingPRAScalesView {
            ...
            productiveScaleView.LoadSliderViews("PRODUCTIVITY", "I don’t work when I travel", "I used my trip to get stuff done!","Taking travel time to get things done, not only for work or study, but also personal things like managing home or family stuff...")
        ...
        if let RelaxingScaleView = nib.instantiate(withOwner: self, options: nil).first as? OnboardingPRAScalesView {
            ...
            RelaxingScaleView.LoadSliderViews("Mind & Pleasure", "Travelling is not at all relaxing", "When I travel, \nI chill", "Relaxing or having fun, taking travel time to do things like listening to music, resting or meditating, engaging in social media, observing...")
        ...
        if let activityScaleView = nib.instantiate(withOwner: self, options: nil).first as? OnboardingPRAScalesView {
            ...
            activityScaleView.LoadSliderViews("Body & Health", "Exercise is in the gym/field", "Travel time is a great way to feel the burn", "When you walk, cycle or maybe even run on your travels, you’re contributing to your health and good look.")
    ...
    func loadSaveButton(){
        ...
        MotivFont.motivBoldFontFor(text: "SAVE", label: NextButton.titleLabel!, size: 15)
