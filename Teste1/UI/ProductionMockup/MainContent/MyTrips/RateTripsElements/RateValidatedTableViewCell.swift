...
class RateValidatedTableViewCell: UITableViewCell {
    
    var part: FullTripPart?

    @IBOutlet weak var MOTAndDistance: UILabel!
    @IBOutlet weak var MOTImage: UIImageView!
    @IBOutlet weak var PointsReceived: UILabel!
    @IBOutlet weak var ValidatedText: UILabel!
...
//            self.MOTAndDistance.text = leg.correctedModeOfTransport ?? leg.modeOfTransport
//            self.MOTAndDistance.text = leg.getTextFromRealMotCode() ?? leg.correctedModeOfTransport ?? leg.modeOfTransport
            ...
            self.PointsReceived.text = "+(INSERT HERE) pts"
            self.ValidatedText.text = "VALIDATED PRODUCTIVITY \(leg.productivityScore ?? 0) RELAXING \(leg.relaxingScore ?? 0)"
            //        if leg.modeOfTransport == (leg.correctedModeOfTransport ?? leg.modeOfTransport) {
            //            self.AutoDetected.text = "Auto Detected"
            //        } else {
            //            self.AutoDetected.text = "Corrected"
            //        }
            
            //        self.QuetionLabel.text = "validated"
...
//            switch leg.getTextModeToShow() {
            ...
            self.PointsReceived.text = "+(INSERT HERE) pts"
            self.ValidatedText.text = "VALIDATED PRODUCTIVITY \(we.productivityScore ?? 0) RELAXING \(we.relaxingScore ?? 0)"
