...
class WhereDoYouLiveOtherViewController: GenericViewController {

    @IBOutlet var tittleLabel: UILabel!
    
    @IBOutlet weak var countryCityTextBox: UITextField!
    override func viewDidLoad() {
        ...
        MotivFont.ChangeColorOnAttributedStringFromLabel(label: self.tittleLabel, color: MotivColors.WoortiOrangeT1)
    }
    ...
    @IBAction func EndEditing(_ sender: Any) {
        ...
        if user?.country == "" {
            user?.country = countryCityTextBox.text ?? ""
        } else {
            user?.city = countryCityTextBox.text ?? ""
        }
