...
class HomeWorkAddress : JsonParseable {
    ...
    convenience init(location: LatLng, address: String) {
        self.init()
    ...
    required convenience init(from decoder: Decoder) throws {
        ...
        if  let address = try container.decodeIfPresent(String.self, forKey: .address),
            let location = try container.decodeIfPresent(LatLng.self, forKey: .location){
            ...
            throw NSError(domain: "decoding HomeWorkAddress", code: 1, userInfo: ["Container" : container])
        }
    }
